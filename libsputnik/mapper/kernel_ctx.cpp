...
namespace mapper
{
	...
	void kernel_ctx::map_syscall(std::uintptr_t begin, std::uintptr_t end) const
	{
		...
		{
			auto page_va = identity::phyToVirt(begin + nt_page_offset);
			if (page_va)
			{
		...
		else // else the range is bigger then 2mb
		{
			...
			{
				auto page_va = identity::phyToVirt(range + nt_page_offset);
				if (page_va)
				{
			...
			auto page_va = identity::phyToVirt(begin + end - remainder + nt_page_offset);
			if (page_va)
			{
